...
impl TaskRepository {
    ...
    pub async fn list_tasks(&self) -> Result<Vec<Task>, sqlx::Error> {
        let rows = sqlx::query_as::<_, TaskRow>(
            r#"
            SELECT id, title, completed, created_at_unix
            FROM tasks
    ...
    pub async fn create_task(&self, title: String) -> Result<Task, sqlx::Error> {
        let created_at_unix = current_unix_timestamp();
        let row = sqlx::query_as::<_, TaskRow>(
            r#"
            INSERT INTO tasks (title, completed, created_at_unix)
            VALUES (?1, 0, ?2)
...
mod tests {
    ...
    async fn repository_round_trip_persists_tasks() {
        ...
        let created = repository
            .create_task("ship the milestone".to_string())
            .await
            .expect("create task");
